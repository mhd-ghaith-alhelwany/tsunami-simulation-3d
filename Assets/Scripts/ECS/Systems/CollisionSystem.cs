    ...
    protected override void OnUpdate()
    {
        ...
        Entities.ForEach((ref SphParticle pi, ref Translation translation) => {
            for(int i = 0; i < boxes.Length; i++){
                ...
                else if(pi.velocity.y < 0 && box.isColliding(new float3(pi.position.x, pi.position.y - EPS, pi.position.z))){
                    pi.position = new float3(pi.position.x, box.center.y + (box.size.y/2) + EPS, pi.position.z);
                    pi.velocity *= BOUND_DAMPING;
                }
                else if(pi.velocity.x > 0 && box.isColliding(new float3(pi.position.x + EPS, pi.position.y, pi.position.z))){
                ...
                else if(pi.velocity.z > 0 && box.isColliding(new float3(pi.position.x, pi.position.y, pi.position.z + EPS))){
